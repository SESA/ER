...
void usage(char *command)
{
  ...
  cerr << "                         PB = patch-based label fusion" << endl;
  cerr << "                         CPB = context patch-based label fusion" << endl;
  cerr << "                         CSVM = context SVM" << endl;
  ...
  cerr << "<-output_prob file>      Output the probability map for each class. The probability map can be used for EM or graphcuts segmentation refinement. The output is a single 4D Nifit image. It temporal dimension represents each tissue class." << endl;
  cerr << endl;
  exit(1);
}
...
int main(int argc, char **argv)
{
  ...
  while(argc > 1){
	...
	fusion = new irtkPatchBasedMSDLabelFusion<irtkGreyPixel>;
      ...
      else if(strcmp(method_name, "CPB") == 0){
	fusion = new irtkContextPatchBasedLabelFusion<irtkGreyPixel>;
      }
      else if(strcmp(method_name, "CSVM") == 0){
