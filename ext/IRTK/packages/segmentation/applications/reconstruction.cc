...
int main(int argc, char **argv)
{
  ...
  // More appropriate for neonatal images. Fetal really need masks.
  ...
  average = reconstruction.CreateAverage(stacks,stack_transformations);
  if (debug)
    // Can be used to check if there has been a registration failure.
  ...
  else
  {
    ...
    //exit(1);
    reconstruction.SetSigma(20); //12mm in usage??
  }
  ...
  for (int iter=0;iter<iterations;iter++)
  {
	...
	else
	{
	  ...
	  else
	  {
        ...
        // Last iteration or packages not specified.
        reconstruction.SliceToVolumeRegistration();
