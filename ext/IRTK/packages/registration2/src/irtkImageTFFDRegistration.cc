...
void irtkImageTFFDRegistration::UpdateSource()
{
  ...
  IRTK_END_TIMING("irtkImageTFFDRegistration::UpdateSource");
}
...
void irtkImageTFFDRegistration::UpdateSourceAndGradient()
{
  ...
  IRTK_END_TIMING("irtkImageTFFDRegistration::UpdateSourceAndGradient");
}
...
{
  if (_affd->GetZ() == 1) {
    return -_affd->Bending() / double(2.0*_affd->GetX()*_affd->GetY()*_affd->GetT());
  } else {
    return -_affd->Bending() / double(3.0*_affd->GetX()*_affd->GetY()*_affd->GetZ()*_affd->GetT());
  }
}
...
void irtkImageTFFDRegistration::SmoothnessPenaltyGradient(double *gradient)
{
  ...
  if (_affd->GetZ() == 1) {
    norm = (double(_target->GetNumberOfVoxels()) / double(2.0*_affd->GetX()*_affd->GetY()*_affd->GetT()));
  } else {
    norm = (double(_target->GetNumberOfVoxels()) / double(3.0*_affd->GetX()*_affd->GetY()*_affd->GetZ()*_affd->GetT()));
  }
