...
{
  // _data covers 8 control points outside the image region (4 on the left and 4 on the right).
  // The control points in the range [left-2, right+2] contribute to the FFD evaluation.
  if ((i < -2) || (i >= _x+2) || (j < -2) || (j >= _y+2) || (k < -2) || (k >= _z+2)) {
    cerr << "irtkFreeFormTransformation3D::Get: No such dof" << endl;
    exit(1);
  }
  x = _data[k][j][i]._x;
