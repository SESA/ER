...
/* Interpolates displacements: This function takes a set of displacements
   ...
   displacements. Interpolate means finding the B-spline coefficients which
   exactly recover the values at the control points.

   It implements Box 2 in this paper:
   [1] Michael Unser. Splines: A Perfect Fit For Signal/Image Processing.
   IEEE Signal Processing Magazine, 1999.
 
   ComputeCoefficients2D or 3D computes the B-spline coefficients by separable
   filtering along each dimension. The code for filtering is located at
   ConvertToInterpolationCoefficients(), which is a cascade of a causal filter and
   an anti-causal filter. The initial condition of the filter is determined
   by assuming the mirror-symmetric boundary condition for the signal,

   mirror-symmetric condition
   s(-k) = s(k)
   s(N-1+k) = s(N-1-k)

   Also, the signal is periodic.

   After the computation, we need to pad the B-spline coefficients for x,y,z < 0
   or x,y,z > end-1 due to the mirror-symmetric boundary assumption. Otherwise,
   FFD2D or FFD3D will not correctly recover the displacements near the boundary.
*/
void irtkBSplineFreeFormTransformation3D::Interpolate(const double* dxs, const double* dys, const double* dzs)
{
  ...
  if (_z == 1) {
    ...
    for (int y = -2; y < _y + 2; y++) {
      for (int x = -2; x < _x + 2; x++) {
	int x2 = x;
	int y2 = y;
	x2 = (x2 < 0)? -x2 : x2;
	x2 = (x2 > (_x-1))? (2*_x - 2 - x2) : x2;
	y2 = (y2 < 0)? -y2 : y2;
	y2 = (y2 > (_y-1))? (2*_y - 2 - y2) : y2;

	_data[0][y][x]._x = xCoeffs(x2, y2, 0);
	_data[0][y][x]._y = yCoeffs(x2, y2, 0);
      }
    }
  } else {
    ComputeCoefficients3D(dxs, dys, dzs, xCoeffs, yCoeffs, zCoeffs);

    for (int z = -2; z < _z + 2; z++) {
      for (int y = -2; y < _y + 2; y++) {
	for (int x = -2; x < _x + 2; x++) {
	  int x2 = x;
	  int y2 = y;
	  ...
	  y2 = (y2 > (_y-1))? (2*_y - 2 - y2) : y2;
	  z2 = (z2 < 0)? -z2 : z2;
	  z2 = (z2 > (_z-1))? (2*_z - 2 - z2) : z2;

	  _data[z][y][x]._x = xCoeffs(x2, y2, z2);
	  _data[z][y][x]._y = yCoeffs(x2, y2, z2);
	  _data[z][y][x]._z = zCoeffs(x2, y2, z2);
	}
      }
    }
  }
